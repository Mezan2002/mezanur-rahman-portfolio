  ...
  return (
    ...
    >
      ...
      <article className="px-6 md:px-12">
        ...
        {/* Content */}
        <div className="prose prose-invert prose-lg max-w-none text-white blog-reveal prose-headings:font-syne prose-headings:font-bold prose-headings:text-white prose-a:text-primary hover:prose-a:text-white prose-strong:text-white prose-blockquote:border-primary prose-blockquote:bg-white/5 prose-blockquote:py-2 prose-blockquote:px-4 prose-blockquote:rounded-r-lg prose-code:text-primary prose-code:bg-white/10 prose-code:px-2 prose-code:rounded prose-pre:bg-black/50 prose-pre:border prose-pre:border-white/10 prose-p:text-white! prose-p:text-white">
          {parse(blog.content || "")}
        </div>
          ...
          <div className="border-t border-white/10 mt-16 pt-8 blog-reveal">
            <div className="flex flex-wrap gap-2">
              {blog.tags.map((tag, i) => (
                <span
                  key={i}
                  className="px-3 py-1 bg-white/5 rounded-full text-xs font-mono text-white uppercase tracking-wider"
                >
                  #{tag.trim()}
                </span>
              ))}

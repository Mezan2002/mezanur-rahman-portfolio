...
export default function ProjectDetails() {
    ...
    const fetchProject = async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          // Small delay to ensure DOM is ready before refresh
          setTimeout(() => {
  ...
  useGSAP(
    () => {
      ...
      const tl = gsap.timeline({
        delay: 0.5, // Buffer for template transition
        onStart: () => {
          ScrollTrigger.refresh();
        },
      });
      ...
      gsap.utils.toArray(".grid-gallery-item").forEach((item) => {
        // Entrance Reveal
        gsap.from(item, {
          y: 60,
          opacity: 0,
  ...
  return (
    <main
      ref={containerRef}
      className="bg-dark-background min-h-screen text-white pt-28 pb-20 overflow-x-hidden"
    >
      <div className="px-6 md:px-12">
        {loading ? (
          ...
          <div className="min-h-[60vh] flex flex-col items-center justify-center text-center">
            <Link
              href="/work"
              className="flex items-center gap-2 text-white hover:text-gray-400 transition-colors uppercase tracking-widest text-[10px] font-bold mb-12"
            >
              <ArrowLeft size={16} /> Back to Work
            </Link>
            <h1 className="text-4xl font-black font-syne uppercase mb-4">
              Project Not Found
            </h1>
            <p className="text-gray-500 font-mono text-sm uppercase tracking-widest mb-12">
              Error_Signal: {error || "Empty_Response"}
            </p>
            <Link
              href="/work"
              className="px-8 py-3 bg-white text-black font-bold font-syne uppercase text-xs rounded-full hover:scale-105 transition-transform"
            >
              View All Projects
            </Link>
          </div>
          ...
          <>
            ...
            <div className="flex items-center gap-4 mb-16 reveal-text opacity-0">
              <Link
                href="/work"
            ...
            <div className="mb-20">
              <div className="overflow-hidden">
                <h1 className="reveal-text opacity-0 text-[10vw] font-black font-syne uppercase leading-[0.9] mb-8 text-white">
                  {project.title}
                </h1>
              </div>

              {project.subtitle && (
                <p className="reveal-text opacity-0 text-2xl text-gray-400 mb-8 font-medium">
                  {project.subtitle}
                </p>
              )}

              <div className="grid grid-cols-1 md:grid-cols-4 gap-8 border-t border-white/20 pt-8 reveal-text opacity-0">
                <div>
            ...
            {/* Hero Image */}
            <div className="relative w-full h-full rounded-xl overflow-hidden mb-24 reveal-image opacity-0">
              <Image
                src={thumbnail}
                alt={project.title}
                width={1920}
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-32">
              <div className="md:col-span-4 space-y-20">
                  ...
                  <div className="reveal-text opacity-0">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {technologies.map((tech, idx) => {
                        ...
                        const cat =
                          typeof tech === "object" ? tech.category : null;
                        return (
                          <div
                            key={idx}
                ...
                {project.technicalChallenges && (
                  <div className="reveal-text opacity-0 relative pl-6 border-l border-white/5">
